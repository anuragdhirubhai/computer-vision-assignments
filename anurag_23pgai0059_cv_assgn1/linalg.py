...
def complicated_matrix_function(M, a, b):
    ...
    out =  dot_product(M, a.T)* dot_product(M, a.T)  #complicated_matrix_function
    # pass
    ### END YOUR CODE
...
def eigen_decomp(M):
    ...
    v = None
    ### YOUR CODE HERE
    w, v = np.linalg.eig(M) #eigen_decomp
    # pass
    ### END YOUR CODE
    return w, v
...
def euclidean_distance_native(u, v):
    ...
    ### YOUR CODE HERE
    # return sum([(i-j)**2 for (i, j) in zip(u,v)])**0.5  #euclidean_distance_native
    return (sum([(j-i)**2 for (j, i) in zip(u,v)])**0.5)  #euclidean_distance_native
    # pass
    ### END YOUR CODE
...
def euclidean_distance_numpy(u, v):
    ...
    ### YOUR CODE HERE
    return (np.sum((v-u)**2)**0.5)  #euclidean_distance_numpy
    # pass
    ### END YOUR CODE
...
def get_eigen_values_and_vectors(M, k):
    ...
    Returns:
        ...
        eigenvectors: list of length k containing the top k eigenvectors
            of shape (m,)
    """
    eigenvalues = [] #empty array
    eigenvectors = [] #empty array
    ### YOUR CODE HERE
    w, v = eigen_decomp(M)
    sort_idx = np.argsort(np.abs(w))[::-1] # it will returns descending indices of the sorted array
